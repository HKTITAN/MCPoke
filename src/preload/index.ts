...
function unwrap<T>(p: Promise<IpcResult<T>>): Promise<T> {
  return p.then((r) => {
    if (r && typeof r === 'object' && 'ok' in r) {
      ...
      throw new IpcBridgeError((r as { error: string }).error, (r as { code?: string }).code)
    }
    return p as T
  })
}
...
const api: ElectronApi = {
  ...
  fetchMarketplace: (search) =>
    unwrap(ipcRenderer.invoke(IPC.marketplaceFetch, search) as Promise<IpcResult<McpRegistryEntry[]>>),
  ...
  onState: (cb) => {
    ...
    return () => { ipcRenderer.removeListener(IPC.onState, fn) }
  },
  onAuthChanged: (cb) => {
    const fn = (_: unknown, a: AuthViewModel) => cb(a)
    ipcRenderer.on(IPC.onAuth, fn)
    return () => { ipcRenderer.removeListener(IPC.onAuth, fn) }
  },
  onLoginCode: (cb) => {
    const fn = (_: unknown, code: { userCode: string; loginUrl: string }) => cb(code)
    ipcRenderer.on(IPC.onLoginCode, fn)
    return () => { ipcRenderer.removeListener(IPC.onLoginCode, fn) }
  },
  onAuthError: (cb) => {
    const fn = (_: unknown, error: AuthErrorEvent) => cb(error)
    ipcRenderer.on(IPC.onAuthError, fn)
    return () => { ipcRenderer.removeListener(IPC.onAuthError, fn) }
  },
  onLogs: (cb) => {
  ...
  openMcpDocs: () => { void ipcRenderer.invoke('mcpoke:ref:mcp') },
  openPokeNpm: () => { void ipcRenderer.invoke('mcpoke:ref:poke') },
  openMcpInspector: (url: string) => { void ipcRenderer.invoke('mcpoke:ref:inspector', url) },
